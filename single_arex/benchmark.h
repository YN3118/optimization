...
// Griewank_shift
double Griewank_shift(vector<double> &x)
{
    double sum1 = 0.0;
    double sum2 = 1.0;
    for (int i = 0; i < x.size(); i++)
    {
        sum1 += pow(x[i] - param.shift[i], 2);
        sum2 = sum2 * cos((x[i] - param.shift[i]) / (i + 1));
    }
    return 1.0 + (sum1 / 4000) - sum2;
}

// xが正の範囲で関数値を2倍にする
double Rastrigin_shaped(vector<double> &x)
{
    double sum = 0.0;
    int count = 0;
    for (int i = 0; i < x.size(); i++)
    {
        if (x[i] > 0.0)
        {
            sum = sum + pow(2 * x[i], 2) - 10 * cos(2 * pi * 2 * x[i]);
        }
        else
        {
            sum = sum + pow(x[i], 2) - 10 * cos(2 * pi * x[i]);
...
double f_value(int i, vector<double> &x)
{
    ...
    if (i == 4)
        return Schwefel(x);
    if (i == 5)
        return Rosenbrock_chain(x);
    if (i == 6)
        return Ellipsoid(x);
    if (i == 7)
        return Bohaxhevsky(x);
    if (i == 8)
        return Griewank(x);
    if (i == 9)
        return Rastrigin_shift(x);
    if (i == 10)
        return Rosenbrock_shift(x);
    if (i == 11)
